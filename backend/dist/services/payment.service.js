...
export async function createShopierPayment(userId, orderId) {
    const order = await prisma.order.findUnique({ where: { id: orderId }, include: { items: true } });
    if (!order)
        throw new HttpError(404, 'Order not found');
    if (order.userId !== userId)
        throw new HttpError(403, 'You cannot pay this order');
...
export async function markShopierCallback(orderId, status, paymentReference) {
