...
export const discordCallback = asyncHandler(async (req, res) => {
  ...
  if (!code) throw new HttpError(400, 'Missing Discord authorization code')
  ...
  if (!tokenResponse.ok) throw new HttpError(401, 'Discord token exchange failed')
  ...
  if (!profileResponse.ok) throw new HttpError(401, 'Discord profile fetch failed')
